      ...
      (err, results) => {
        ...
        if (results.rows.length > 0) {
          ...
          pool.query(
            ...
            [createusername, createemail, hashedpassword],
            (err, results) => {
              if (err) {
                throw err;
              }
          ...
          pool.query(
            `INSERT INTO nutrition (email) VALUES ($1)`,
            [createemail],
            (err, results) => {
              if (err) {
                throw err;
...
app.post("/profile", (req, res) => {
  ...
  pool.query(
    "UPDATE nutrition SET user_journal=$1 WHERE email=$2",
    [user_journal, req.user.email],
    (err, results) => {
      if (err) {
        throw err;
      }
...
app.post("/nutritioncalc", function (req, res) {
  ...
  pool.query(
    `UPDATE nutrition SET calorie_intake=\'{${breakfast},${lunch},${dinner},${snack1},${snack2}}\' WHERE email=$1`,
    [req.user.email],
    (err, results) => {
      if (err) {
        throw err;
...
app.post("/BMRcalc", function (req, res) {
  ...
  pool.query(
    "UPDATE nutrition SET bmr=$1 WHERE email=$2",
    [BMR, req.user.email],
    (err, results) => {
      if (err) {
        throw err;
...
app.post("/TDEEcalc", function (req, res) {
  ...
  // use update to modify existing row
  pool.query(
    "UPDATE nutrition SET tdee=$1 WHERE email=$2",
    [TDEE, req.user.email],
    (err, results) => {
      if (err) {
        throw err;
...
app.get("/fitness", function (req, res) {
  ...
  var query =
    "Select activityName, date, duration, type, difficulty from fitness WHERE email='" +
    user_email +
    "';";
  console.log("query: " + query);
  var activityName;
...
app.get("/sleep", function (req, res) {
  ...
  console.log("User email: " + user_email);
  var query =
    "SELECT sleeptype, date, yesno, starttime, endtime FROM sleepinfo WHERE email='" +
    user_email +
    "';";
  console.log("query: " + query);
